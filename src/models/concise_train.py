...
parser.add_argument('--model', type=str, required='nb', choices=['nb', 'lr', 'dt'], help='Name and directory to store trained model')
...
def extract_features(data, preprocess):
    ...
    for row in data:
        ...
        content = title + short_description + category_lvl_1 + category_lvl_2 +  category_lvl_3

        if preprocess:
            content = pp.process_data(content)
